...
namespace MathLibrary
{
    public class Calculator
    {
        public static int Calculate(string expression)
        {
            if (expression == "")
            {
                throw new CalculateExceptionStringIsEmpty("Missing expression input!");
            }

            try
            {
                int result = Counting(ConvertExpression(PreparatingExpression(expression)));
                return result;
            }
            catch (DivideByZeroException)
            {
                throw new CalculateExceptionDivideByZero("Division by zero is prohibited!");
            }
            catch (Exception)
            {
                throw new CalculateExceptionInvalidExpression("Invalid expression!");
            }
        }
